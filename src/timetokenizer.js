...
var timetokenizer = {
     ...
     * @contributors Joshua Gammage
     */
    isNumberStart(charCode) {
        return this.isDecDigit(charCode) || this.isSign(charCode) ||
            charCode === 46;
    },
    /**
     * @description Utility method for determining whether a given character is
    ...
    parseString() {
        ...
        while (!this.end()) {
            let charCode = dat.time.charCodeAt(dat.ind);
    ...
    parseIdentifier() {
        ...
        let keywordInd = keywords.indexOf(name.toUpperCase());
    ...
    parseDecStart() {
        ...
        while (!this.end() && this.isDecDigit(dat.time.charCodeAt(dat.ind)) &&
            dat.time[dat.ind] !== ".") {
    ...
    parseDecEnd() {
        ...
        while (!this.end() &&
            (this.isDecDigit(dat.time.charCodeAt(dat.ind)) ||
                dat.time[dat.ind] === ".")) {
            decEnd += dat.time[dat.ind];
            ++dat.ind;
        }
        
        let charCode2 = dat.time.charCodeAt(dat.ind + 1);
        
        if (["e", "E"].indexOf(dat.time[dat.ind]) >= 0 &&
